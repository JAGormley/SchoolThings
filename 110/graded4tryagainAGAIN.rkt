...
#;
(define (next-count c)
  (make-countdown 500 400 10))


#;
(define (fn-for-countdown c)  ;Template from Countdown
  (... (countdown-x c)
       (countdown-y c)
       (countdown-i c)))
...
#;
(define (render c)      ; template from Countdown
  (... (countdown-x c)
       (countdown-y c)
       (countdown-i c)))
...
;; clicking the mouse on the screen creates new Countdown
