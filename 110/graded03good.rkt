...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname graded03good) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
;; Problem Set 3:
...
;; no tests for main function
 ...
 (define (main hd)
  (big-bang hd
            ...
            (on-key handle-key)))        ; KeyEvent -> HelloDown
...
(define (render-hellodown hd)
  (place-image (rotate (remainder (hellodown-rotate hd) 360)
                       (text "hello" 30 "black")) 
               CTR-X 
               (hellodown-y hd) 
               MTS))
...
(check-expect (handle-key (make-hellodown 20 60) " ") (make-hellodown 0 0))
(check-expect (handle-key (make-hellodown 20 60) "a") (make-hellodown 20 60))

#;
(define (handle-key hd ke) hd)       ;stub

#;
(define (handle-key hd ke)          ;templated according to KeyEvent
  (cond [(key=? ke " ") (make-hellodown 10 10)]
        [else hd]))
